...
# 统计未获取到商品详情信息数据条数
noget_detail = 'grep -o "未获取到对应的详情信息,url:" ' + BASE_DIR + '/log/syslog|wc -l'
...
# 已入库总数
success_count = dbservice.db['products'].find().count()
...
with open(email_content, 'w', encoding='utf-8') as f:
    ...
    f.write('截止到当前时间 {}  BestBuy数据总入库：{} 条\n'.format(time.strftime("%Y-%m-%d %H:%M:%S", time.localtime()),
                                                     str(success_count)))

    # f.write('截止到当前时间 {} Newegg 抓取数据量整体进度完成：{:.2%}'.format(time.strftime("%Y-%m-%d %H:%M:%S", time.localtime()),
